...
export default function WithdrawTab(props: WithdrawTabProps) {
  ...
  const maxSharesString =
    (maxShares) ?
      maxShares.div(String1E(18))
        .toFixed(18)
      : undefined;
  ...
  useEffect(() => {
    ...
    if (estimated0Label.length > 9) {
      estimated0Label = estimated0.div(String1E(poolStats.token0Decimals))
        .toExponential(4)
    }

    if (estimated1Label.length > 9) {
      estimated1Label = estimated1.div(String1E(poolStats.token1Decimals))
        .toExponential(4)
    }
  ...
  }, [isTransactionPending, maxShares, poolStats, ratioChange, sharesAmount, sharesBig]);
    ...
    switch (buttonState) {
      case ButtonState.READY:
        return () => {
          ...
          withdraw(signer, props.poolData.poolAddress, shares, Number(ratioChange), poolStats, (receipt) => {
      ...
      <div className='w-full'>
        ...
        <div className='w-full'>
          <PrimaryButton className='w-full py-2 mt-2'
                         ...
                         disabled={([ButtonState.INSUFFICIENT_SHARES,
