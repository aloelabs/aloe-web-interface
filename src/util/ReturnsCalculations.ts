...
type PoolReturns = PoolPerformanceSnapshot[];
type TokenReturns = TokenPerformanceSnapshot[];

export function calculateReturns(poolReturns: PoolReturns, token0Returns: TokenReturns, token1Returns: TokenReturns) {
    if (poolReturns.length !== token0Returns.length || poolReturns.length !== token1Returns.length) {
        // TODO: fail more gracefully than this
        throw new Error("Arrays should have identical lengths and matching timestamps!");
    }
