interface PoolPerformanceSnapshot {
    timestamp: number; // TODO this should be a number like `TokenPerformanceSnapshot`, but Matt hasn't standardized the API yet
    block_number: number;
    pool_address: string;
    chain_id: number;
    inventory0: number;
...
export function calculateReturns(poolReturns: PoolReturns, token0Returns: TokenReturns, token1Returns: TokenReturns) {
    ...
    const [matchingPoolReturns, matchingToken0Returns, matchingToken1Returns] = intersectionOfReturns(poolReturnsCopy, token0Returns, token1Returns);
    ...
    const initialPoolTVL = (matchingPoolReturns[initialIndex].inventory0 * initialP0) + (matchingPoolReturns[initialIndex].inventory1 * initialP1);
