...
export async function withdraw(signer: Signer, poolAddress: string, shares: Big, ratioChange: number, poolStats: BlendPoolStats, completionCallback: (receipt?: ContractReceipt) => void): Promise<void> {
  ...
  try {
    const transactionResponse = await tokenContract.withdraw(shares.toFixed(0), amount0Min.toFixed(0), amount1Min.toFixed(0));
