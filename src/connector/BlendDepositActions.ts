import { BLOCKS_TO_WAIT, UINT256_MAX } from '../data/constants/Values';
...
export async function approve(signer: Signer, tokenAddress: string, poolAddress: string, completionCallback: (receipt?: ContractReceipt) => void, amount: string = UINT256_MAX): Promise<void> {
...
export async function mintWeth(signer: Signer, amount: Big, completionCallback: (receipt?: ContractReceipt) => void): Promise<void> {
...
export async function deposit(signer: Signer, poolAddress: string, amount0Max: Big, amount1Max: Big, ratioChange: number, completionCallback: (receipt?: ContractReceipt) => void): Promise<void> {
  ...
  try {
    const transactionResponse = await blendContract.deposit(amount0Max.toFixed(0), amount1Max.toFixed(0), amount0Min.toFixed(0), amount1Min.toFixed(0));
    const receipt = await transactionResponse.wait(BLOCKS_TO_WAIT);
    completionCallback(receipt);
  } catch (e) {
    // User probably rejected in MetaMask or wallet
